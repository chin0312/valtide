...
def _patch_sources(monkeypatch, *, quote, bar, ref):
    ...
    monkeypatch.setattr(live.equity, "get_last_trusted_close", lambda *a, **k: bar)
...
def test_falls_back_to_stale_reference_when_xperp_down(monkeypatch):
    _patch_sources(
        monkeypatch,
        quote=TokenQuote(price=185.1, source="dexscreener", ts=datetime.now(UTC)),
        bar=RawEquityBar(
    ...
    snap = build_live_snapshot()
    assert snap.reference_under_test == 180.0
    assert snap.reference_under_test_source == "stale_nvda"
