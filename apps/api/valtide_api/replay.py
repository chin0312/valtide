...
from valtide_api.quant_runtime import ModelArtifact, estimate, load_artifact
...
def run_inference(
    ...
    est = estimate(snapshot, prior.m, prior.P, artifact=art)
    result = validate(
        snapshot,
        est,
...
def replay(
    ...
    """
    art = artifact or load_artifact()
    state = KalmanState(m=art.m0, P=art.P0, last_ts=None)
...
def seed_state_from_artifact(artifact: ModelArtifact | None = None) -> KalmanState:
    """Fresh Kalman state at the artifact's trained initial state."""
    art = artifact or load_artifact()
    return KalmanState(m=art.m0, P=art.P0, last_ts=None)
